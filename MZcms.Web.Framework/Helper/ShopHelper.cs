...
namespace MZcms.Web.Framework
{
    ...
    public class ShopHelper
    {
        ...
        public ShopHelper(string app_key)
        {
            ...
            if (shopappinfo == null)
            {
                throw new MZcmsApiException(Hishop.Open.Api.OpenApiErrorCode.Invalid_App_Key, "app_key");
            }
            if (shopappinfo.IsEnable != true)
            {
                throw new MZcmsApiException(Hishop.Open.Api.OpenApiErrorCode.System_Error, "function not open");
            }
            _AppSecreate = shopappinfo.AppSecreat;
            if (string.IsNullOrWhiteSpace(_AppSecreate))
            {
                throw new MZcmsApiException(Hishop.Open.Api.OpenApiErrorCode.Insufficient_ISV_Permissions, "not set app_secreat");
            }
            ...
            if (manage == null)
            {
                throw new MZcmsException("店铺管理信息有误，请管理员修正");
            }
