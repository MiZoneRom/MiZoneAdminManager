...
namespace MZcms.Web.Framework
{
    public class ApiHelper
    {
        ...
        public static void CheckBaseParamsAndSign(SortedDictionary<string, string> data)
        {
            if (data == null)
            {
                throw new MZcmsApiException(ApiErrorCode.System_Error, "no params");
            }
            if (data.Count < 1)
            {
                throw new MZcmsApiException(ApiErrorCode.System_Error, "no params");
            ...
            {
                throw new MZcmsApiException(ApiErrorCode.Missing_Timestamp, "timestamp");
            }
            ...
            {
                throw new MZcmsApiException(ApiErrorCode.Invalid_Timestamp, "timestamp");
            }
            if (!data.TryGetValue("sign", out sign))
            {
                throw new MZcmsApiException(ApiErrorCode.Missing_Signature, "sign");
            }
            ...
            {
                throw new MZcmsApiException(ApiErrorCode.Invalid_Signature, "sign");
            }
        }
        ...
        public static WxAppletUserInfo GetAppletUserInfo(HttpRequestMessage curRequest)
        {
            HttpContextBase context = (HttpContextBase)curRequest.Properties["MS_HttpContext"];//获取传统context
