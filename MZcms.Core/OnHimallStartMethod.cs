...
namespace MZcms.Core
{
    ...
    public class OnMZcmsStartMethodAttribute:Attribute
    {
        /// <summary>
        /// 没有返回值的空参数签名
        /// </summary>
        ...
        public OnMZcmsStartMethodAttribute(Type type, string methodName, int order = 0)
        {
        ...
        static void StartImp()
        {
            ...
            var thisType = typeof(OnMZcmsStartMethodAttribute);
            ...
            var MZcmsStartMethodAttributes = allAttributes.Select(item => item as OnMZcmsStartMethodAttribute).OrderBy(item => item.Order);
            foreach (var atrribute in MZcmsStartMethodAttributes)
            {
